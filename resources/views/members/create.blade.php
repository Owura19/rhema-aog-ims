...
<div style="margin-bottom:20px; display:flex; align-items:center; justify-content:space-between;">
    <div>
        ...
        <h2 style="font-size:20px; font-weight:700; color:#1e293b; margin-top:4px;">Add New Member</h2>
    </div>
</div>
    ...
    <div class="card-body">
        <div class="grid-3">
            ...
            <div>
                ...
                <input type="text" name="other_name" value="{{ old('other_name') }}" class="form-control" placeholder="Middle name (optional)">
            </div>
            ...
            <div>
                ...
                <input type="text" name="occupation" value="{{ old('occupation') }}" class="form-control" placeholder="e.g. Teacher, Engineer">
            </div>

            <div>
                <label class="form-label">Employer</label>
                <input type="text" name="employer" value="{{ old('employer') }}" class="form-control" placeholder="Employer / Company">
            </div>
    ...
    <div class="card-body">
        <div class="grid-3">
            ...
            <div>
                ...
                <input type="text" name="alt_phone" value="{{ old('alt_phone') }}" class="form-control" placeholder="Alternative phone">
            </div>

            <div>
                <label class="form-label">Email Address</label>
                <input type="email" name="email" value="{{ old('email') }}" class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" placeholder="email@example.com">
                @error('email')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div style="grid-column:span 2;">
                <label class="form-label">Residential Address</label>
                <input type="text" name="residential_address" value="{{ old('residential_address') }}" class="form-control" placeholder="House number, street, area">
            </div>

            <div>
                <label class="form-label">Digital Address (Ghana Post GPS)</label>
                <input type="text" name="digital_address" value="{{ old('digital_address') }}" class="form-control" placeholder="e.g. GA-123-4567">
            </div>
    ...
    <div class="card-body">
        <div class="grid-3">
            ...
            <div>
                ...
                <select name="membership_status" class="form-control {{ $errors->has('membership_status') ? 'is-invalid' : '' }}">
                    ...
                    @foreach(['Active','Inactive','Visitor','Transferred','Deceased'] as $status)
                        <option value="{{ $status }}" {{ old('membership_status', 'Active') == $status ? 'selected' : '' }}>{{ $status }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="member_type" class="form-control {{ $errors->has('member_type') ? 'is-invalid' : '' }}">
                    ...
                    @foreach(['Full Member','Associate','Visitor'] as $type)
                        <option value="{{ $type }}" {{ old('member_type', 'Full Member') == $type ? 'selected' : '' }}>{{ $type }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="date_joined" value="{{ old('date_joined') }}" class="form-control">
            </div>

            <div>
                <label class="form-label">Date Baptized</label>
                <input type="date" name="date_baptized" value="{{ old('date_baptized') }}" class="form-control">
            </div>
            ...
            <div>
                ...
                <select name="family_role" class="form-control">
                    ...
                    @foreach(['Head','Spouse','Child','Other'] as $role)
                        <option value="{{ $role }}" {{ old('family_role') == $role ? 'selected' : '' }}>{{ $role }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="number" name="fingerprint_id" value="{{ old('fingerprint_id') }}" class="form-control" placeholder="ZKTeco user ID">
